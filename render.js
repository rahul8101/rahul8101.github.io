  ...
  var path="M"+x1+","+y1+" L"+x2+","+y2+" L"+x2+","+(y2-hgt)+" L"+x1+","+(y1-hgt)+" z";
    ...
    Render=function(){
     ...
     console.log(typeof obj);
      ...
      var base_top=paper.path("M80,"+(80+k*5)+" L"+(80+Number(obj.width)-130)+","+(80+k*5)+" L"+(80+Number(obj.width)-140)+","+(95+k*5)+" L70,"+(95+k*5)+" z");
     ...
     //-----------------
      ...
      for(i=0;i<length;i++){
         face_set[i]=paper.set();
          ...
          //------------------------------------------------------
          var top_face=paper.path("M"+((80+block_size*i))+","+(Number(obj.height)-105)+
                       " L"+((80+block_size*i)+15)+","+(Number(obj.height)-120)+
                       " L"+((80+block_size*i)+15+block_size)+","+(Number(obj.height)-120)+
                       " L"+((80+block_size*i)+block_size)+","+(Number(obj.height)-105)+" z");
          top_face.attr({fill:pallete[1],"stroke-width":0});
          top_face.node.setAttribute("number",i);

          //------------------------------------------------------
          var front_face_path=paper.path_generator((80+block_size*i),(Number(obj.height)-105),80+block_size*i+block_size,(Number(obj.height)-105),0);
          ...
          console.log(data1[i]);
          if(data1[i]>=0){
          var side_face_path=paper.path_generator(((80+block_size*i)+block_size),(Number(obj.height)-105),((80+block_size*i)+15+block_size),(Number(obj.height)-120),0);
          }
          else{
          var side_face_path="M"+((80+block_size*i)+block_size)+","+(Number(obj.height)-105)+" L"+
                                   ((80+block_size*i)+15+block_size)+","+(Number(obj.height)-105)+" L"+
                                   ((80+block_size*i)+15+block_size)+","+(Number(obj.height)-120)+
                                   " L"+((80+block_size*i)+block_size)+","+(Number(obj.height)-105)+" z";
          ...
          for(i=0;i<length;i++){
          var front_face_path=paper.path_generator((80+block_size*i),(Number(obj.height)-105),80+block_size*i+block_size,(Number(obj.height)-105),height[i]);
...
//------------------------------------------------------------------------------------------
          if(data1[i]>=0){
          var side_face_path=paper.path_generator(((80+block_size*i)+block_size),(Number(obj.height)-105),((80+block_size*i)+15+block_size),(Number(obj.height)-120),height[i]);
            }
          else{
          var side_face_path="M"+((80+block_size*i)+block_size)+","+(Number(obj.height)-105)+" L"+
                                   ((80+block_size*i)+15+block_size)+","+(Number(obj.height)-105)+" L"+
                                   ((80+block_size*i)+15+block_size)+","+(Number(obj.height)-120-height[i])+
                                   " L"+((80+block_size*i)+block_size)+","+(Number(obj.height)-105-height[i])+" z"; 
            }
//------------------------------------------------------------------------------------------
          ...
          face_set[i].mouseover(function(e){
              ...
              if(flag){
                 ...
                 tool_tip=paper.rect((e.clientX)+5,e.clientY,130,20,3).attr({fill:"black",opacity:0.8});
                     text=paper.text((e.clientX)+10,e.clientY+10,tool_text1[this.node.getAttribute("number")]).attr({fill:"white","text-anchor":"start"});
                 m=0;
                   }                 
               flag=0;
              }
      ...
      //-------------------------------------------------------------------------------------------------------------------
           var dx=80+(Number(obj.width)-130)/3;
            ...
            var legend_bar_rect1=paper.path("M"+dx+","+(Number(obj.height)+120)+" L"+dx+","+(Number(obj.height)+105)+" L"+(dx+5)+","+(Number(obj.height)+105)+" L"+(dx+5)+","+(Number(obj.height)+120)+" z").attr({fill:"#6a5acd","stroke-width":0});
           var legend_bar_rect2=paper.path("M"+(dx+6)+","+(Number(obj.height)+120)+" L"+(dx+6)+","+(Number(obj.height)+100)+" L"+(dx+11)+","+(Number(obj.height)+100)+" L"+(dx+11)+","+(Number(obj.height)+120)+" z").attr({fill:"#6a5acd","stroke-width":0});
           var legend_bar_rect3=paper.path("M"+(dx+12)+","+(Number(obj.height)+120)+" L"+(dx+12)+","+(Number(obj.height)+110)+" L"+(dx+17)+","+(Number(obj.height)+110)+" L"+(dx+17)+","+(Number(obj.height)+120)+" z").attr({fill:"#6a5acd","stroke-width":0});
           var legend_bar_text=paper.text((dx+21),(Number(obj.height)+115),obj.dataSource.dataset[0].seriesName).attr({"text-anchor":"start","font-size":13});
           var bar_legend=paper.set(legend_bar_rect1,legend_bar_rect2,legend_bar_rect3,legend_bar_text);
           
           var legend_line_path=paper.path("M"+(dx+110)+","+(Number(obj.height)+115)+"h30,0").attr({"stroke":"#32cd32","stroke-width":3});
           var legend_line_circle=paper.circle((dx+125),(Number(obj.height)+115),5).attr({fill:"white"});
           var legend_line_text=paper.text((dx+142),(Number(obj.height)+115),obj.dataSource.dataset[1].seriesName).attr({"text-anchor":"start","font-size":13});
